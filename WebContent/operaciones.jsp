...
<body>
	<%
		/**
		 * Archivo jsp que las operaciones de la base de datos de la web
		 * 
		 * @author Ian Lopez
		 * @version 19/05/2020
		 */
	...
	<%
		...
		if (request.getParameter("tipo").equalsIgnoreCase("registroCliente")) {
			...
			if (request.getParameter("nombreCliente").equalsIgnoreCase("")) {
				...
				operacion = "incorrecta";
			} else if (request.getParameter("durSuscripcion").equalsIgnoreCase("")
					&& request.getParameter("tipoSuscripcion").equalsIgnoreCase("Prime")) {
				mensaje = "La duracion de suscripcion con prime del cliente no puede estar vacio, repita la operacion";
				operacion = "incorrecta";
			} else if (!request.getParameter("durSuscripcion").isEmpty()
					&& request.getParameter("tipoSuscripcion").equalsIgnoreCase("Basica")) {
				...
				Cliente cliente = new Cliente(id_cliente, direccionCliente, nombreCliente, apellidoCliente,
						correoCliente, tlfCliente, tipoSuscripcion, durSuscripcion, id_amazon_registro);
		...
		if (request.getParameter("tipo").equalsIgnoreCase("altaProveedor")) {
			...
			if (request.getParameter("nombreProveedor").equalsIgnoreCase("")) {
				...
				id_amazon_registro = Integer.parseInt(request.getParameter("id_amazon"));
				Proveedor proveedor = new Proveedor(id_proveedor, nombreProveedor, direccionProveedor, tlfProveedor,
						correoProveedor);
		...
		if (request.getParameter("tipo").equalsIgnoreCase("altaEmpleado")) {
			if (controladorBD.existeEmpleado(request.getParameter("dniEmpleado")) == true) {
				...
				id_amazon_registro = Integer.parseInt(request.getParameter("id_amazon"));
				Empleado empleado = new Empleado(dniEmpleado, nomEmple, apeEmple, direcEmple, tlfEmple,
						id_amazon_registro);
		...
		if (request.getParameter("tipo").equalsIgnoreCase("altaProduct")) {
			...
			if (request.getParameter("paisProd").equalsIgnoreCase("")) {
				...
				id_departamento = Integer.parseInt(request.getParameter("id_departamento"));
				Producto producto = new Producto(id_producto, nomProdu, paisProd, precio, marca, modelo, especific,
						existencias, id_departamento);
		...
		if (request.getParameter("tipo").equalsIgnoreCase("altaPedido")) {
			...
			if (request.getParameter("fechaRealizada").equalsIgnoreCase("")) {
				...
				operacion = "incorrecta";
			} else if (request.getParameter("estadoPedido").equalsIgnoreCase("Entregado")
					|| request.getParameter("estadoPedido").equalsIgnoreCase("Entregado con Retraso")) {
				if (request.getParameter("fechaEntregada").equalsIgnoreCase("")) {
					...
					Pedido pedido = new Pedido(id_pedido, fechaRealizada, fechaEntrePrev, fechaDespachada,
							fechaEntregada, estadoPedido, review, monto, gastos, id_cliente);
					controladorBD.insertarPedido(pedido, id_producto);
					mensaje = "Operacion realizada exitosamente";

				}
			} else if (request.getParameter("estadoPedido").equalsIgnoreCase("En Camino")
					|| request.getParameter("estadoPedido").equalsIgnoreCase("En Reparto")
					|| request.getParameter("estadoPedido").equalsIgnoreCase("Producto Perdido")) {
				if (!request.getParameter("fechaEntregada").equalsIgnoreCase("")) {
		...
		if (request.getParameter("tipo").equalsIgnoreCase("altaServc")) {
			...
			Servicio_Postventa servPost = new Servicio_Postventa(id_servicio, id_pedido, dniEmpleado, tipoServ,
					estadoServ);
		...
		if (request.getParameter("tipo").equalsIgnoreCase("modCliente")) {
			if (request.getParameter("nomClien").equalsIgnoreCase("")) {
				...
				mensaje = "El telefono del cliente no puede estar vacio, repita la operacion";
				operacion = "incorrecta";
			} else if ((request.getParameter("duracionSuscrip").equalsIgnoreCase("")
					&& request.getParameter("tipoSuscripcion").equalsIgnoreCase("Prime"))
					|| (request.getParameter("duracionSuscrip").equalsIgnoreCase("null")
							&& request.getParameter("tipoSuscripcion").equalsIgnoreCase("Prime"))) {
				mensaje = "La duracion de suscripcion con prime del cliente no puede estar vacio, repita la operacion";
				operacion = "incorrecta";
			} else if (!request.getParameter("duracionSuscrip").isEmpty()
					&& request.getParameter("tipoSuscripcion").equalsIgnoreCase("Basica")) {
				mensaje = "La duracion de suscripcion tiene que estar vacia con basica, repita la operacion";
				operacion = "incorrecta";
